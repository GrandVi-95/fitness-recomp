...
async function getDashboardData() {
  ...
  const [user, nutritionLogs, latestMetric, lastSession] = await Promise.all([
    db.user.findUnique({
  ...
  // Targets — prefer UserSettings.autoProteinGoal calc, fallback to User fields
  ...
  // Next workout — look at the active plan's workouts in order vs last session's dayLabel
  ...
  if (activePlan?.workouts?.length) {
    if (!lastSession) {
      ...
      const idx = activePlan.workouts.findIndex(
  ...
  return (
    <div className="flex flex-col items-center gap-1">
      ...
      <p className="text-[11px] font-semibold">
        {current}<span className="text-slate-500 font-normal">{unit}</span>
      </p>
    </div>
  )
}
...
export default async function DashboardPage() {
  const { userName, targetCalories, targetProtein, todayNutrition, latestWeight, nextWorkout } =
    await getDashboardData()

  const caloriesLeft = targetCalories - todayNutrition.calories
  const proteinLeft  = targetProtein  - todayNutrition.protein
  const isProteinBehind = proteinLeft > targetProtein * 0.3 // > 30 % remaining as "behind"
